/***
 *	
 *	file name : Plot_LepTop_DataMC.cc
 *	Data : 191026
...
{	
	...
	f->GetObject("h_l_VV_mu",h_VV_mu);
	f->GetObject("h_l_ST_mu",h_ST_mu);
	f->GetObject("h_l_QCD_mu",h_QCD_mu);
	f->GetObject("h_l_Data_mu",h_Data_mu);

	f->GetObject("h_l_TT_el",h_TT_el);
	f->GetObject("h_l_DY_el",h_DY_el);
	f->GetObject("h_l_WJets_el",h_WJets_el);
	f->GetObject("h_l_VV_el",h_VV_el);
	...
	*/
	...
	string filename = string(argv[2]) + "_LepTop";	

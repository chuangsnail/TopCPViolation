/**************************************************************************************
 *
 *	File Name : CR_re_btag.cc
 *	Description : store chi2 < 20 sample
 *	Date : 190815~
...
int main(int argc,char* argv[])
{
	...
	int PUI_size = 76;
	char path_PU_file[200] = "/wk_cms2/cychuang/CMSSW_9_4_13/src/TopCPViolation/data/pileupweights_69200.csv"; 
	GetPileUpInfo( path_PU_file , pileupinfo , &PUI_size);

	//insert tight-Muon ID Scale Factors' TH2F*
	TFile* f1 = new TFile("/wk_cms2/cychuang/CMSSW_9_4_13/src/TopCPViolation/data/muIDSF.root");
	TH2F* h_tightMuIDSF;		f1->GetObject("abseta_pt_ratio",h_tightMuIDSF);

	TFile* f2 = new TFile("/wk_cms2/cychuang/CMSSW_9_4_13/src/TopCPViolation/data/muISOSF.root");
	TH2F* h_MuISOSF;			f2->GetObject("abseta_pt_ratio",h_MuISOSF);

	TFile* f3 = new TFile("/wk_cms2/cychuang/CMSSW_9_4_13/src/TopCPViolation/data/muTrgSF.root");
	TH2F* h_MuTrgSF;			f3->GetObject("abseta_pt_ratio",h_MuTrgSF);

	//insert tight-Electron ID Scale Factor TH2F*
	TFile* f4 = new TFile("/wk_cms2/cychuang/CMSSW_9_4_13/src/TopCPViolation/data/elTightIDSF.root");
	TH2F* h_tightElIDSF;		f4->GetObject("EGamma_SF2D",h_tightElIDSF);

	TFile* f5 = new TFile("/wk_cms2/cychuang/CMSSW_9_4_13/src/TopCPViolation/data/elRECOSF.root");
	TH2F* h_ElRECOSF;			f5->GetObject("EGamma_SF2D",h_ElRECOSF);

	TFile* f6 = new TFile("/wk_cms2/cychuang/CMSSW_9_4_13/src/TopCPViolation/data/elTrgSF.root");
	...
	TFile* f7 = new TFile("/wk_cms2/cychuang/CMSSW_9_4_13/src/TopCPViolation/data/beffPlot_WJets_0pt5426.root");
	...
	TH1F* h_Data_mu = new TH1F("h_Data_mu"," ;Hadronic jjb Mass(GeV);Events(No.)",bins_No,hist_min,hist_max);	
	//"(primary title);(X-Axis title);(Y-Axis Title)"
	TH1F* h_Data_el = new TH1F("h_Data_el"," ;Hadronic jjb Mass(GeV);Events(No.)",bins_No,hist_min,hist_max);

	TH1F* h_btag_w = new TH1F("h_btag_w", "",60,0.5,3.5);
	...
	//*******Tool to calculate total event******//
/*
	double t_TT_weight_mu = 0.;		double t_DY_weight_mu = 0.;		double t_WJets_weight_mu = 0.;
	double t_VV_weight_mu = 0.;		double t_ST_weight_mu = 0.;		double t_QCD_weight_mu = 0.;
	double t_TT_weight_el = 0.;		double t_DY_weight_el = 0.;		double t_WJets_weight_el = 0.;
	double t_VV_weight_el = 0.;		double t_ST_weight_el = 0.;		double t_QCD_weight_el = 0.;

	vector<double> t_weight_mu;
	t_weight_mu(t_TT_weight_mu);	t_weight_mu(t_DY_weight_mu);	t_weight_mu(t_WJets_weight_mu);
	t_weight_mu(t_VV_weight_mu);	t_weight_mu(t_ST_weight_mu);	t_weight_mu(t_QCD_weight_mu);
	vector<double> t_weight_el;
	t_weight_el(t_TT_weight_el);	t_weight_el(t_DY_weight_el);	t_weight_el(t_WJets_weight_el);
	t_weight_el(t_VV_weight_el);	t_weight_el(t_ST_weight_el);	t_weight_el(t_QCD_weight_el);
*/
	...
	for(int k=0;k<(int)files_map.size();k++)
	{
		...
		for(int r=0;r<(int)files_map[ Set_name ]->size();r++)
		{
			...
			for(int entry=0;entry<(int)t_entries;++entry)
			{
				...
				if( entry%50000 == 0 )
				{	
					cout << "In entry : " << entry << endl;
				}

				double btag_weight = 0.;
				...
				bool pass_sel_el = Pass_CR_Selected_Electron(leptonInfo,idx_Selected_El);
				...
				if( (pass_sel_mu||pass_sel_el) && !(pass_sel_mu&&pass_sel_el) )
				{
					if(pass_sel_mu)
					{
						pass_veto_el = Pass_CR_Veto_Electron(leptonInfo,idx_Selected_Mu);
						pass_veto_mu = Pass_CR_Veto_Muon(leptonInfo,idx_Selected_Mu);
					}
					if(pass_sel_el)
					{
						pass_veto_el = Pass_CR_Veto_Electron(leptonInfo,idx_Selected_El);
				...
				if( is_data )
				{
					/*	
					if(channel == "mu")
					{	is_pass_HLT = Pass_HLT( evtInfo, HLT_Data_mu );	}
					else if(channel == "el")
					{	is_pass_HLT = Pass_HLT( evtInfo, HLT_Data_el );	}
					*/
					if( r == 0 )
					{	is_pass_HLT = Pass_HLT( evtInfo, HLT_Data_mu );	}
					else if( r == 1 )
				...
				{	
					if(channel == "mu")
					{	is_pass_HLT = Pass_HLT( evtInfo, HLT_MC_mu );	}
					else if(channel == "el")
					{	is_pass_HLT = Pass_HLT( evtInfo, HLT_MC_el );	}
				}
				if( !is_pass_HLT )
				{	continue;	}
				...
				if( (is_mu_channel && !is_el_channel) || (is_el_channel && !is_mu_channel) )
				{
					...
					if(!is_data)
					{
						...
						//
						//in CR , there is no jets pass the btag standard, so all jets used in the nontagged case
						
						vector<int> all_jets;
						for(int q=0;q<(int)sel_b_jets.size();q++)
						{	all_jets.push_back( sel_b_jets.at(q) );	}
						for(int q=0;q<(int)sel_jets.size();q++)
						{	all_jets.push_back( sel_jets.at(q) );	}

						bmgr.Reset_idx_capacity();
						bmgr.Reset_b_tagged_jets_idx();
						bmgr.Set_b_ntagged_jets_idx( all_jets );
						btag_weight = bmgr.Get_Btag_Weight();
						weight *= btag_weight;
						h_btag_w->Fill( btag_weight );
						
						if( entry % 10000 == 0 ){
							cout << "btag_weight : " << btag_weight << endl;
						}
				...
				TLorentzVector b1, j1, j2;
				//TLorentzVector lepton, b2;
	...
	string new_file_name = "CR_rebtag_" + time + ".root";

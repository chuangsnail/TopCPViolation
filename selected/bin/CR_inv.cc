 ...
 *
 *	File Name : CR_inv.cc
 *	Description : add JER , modified ISO in Muon Selected selection , used to data-driven the QCD part
 *	Date : 191014~
...
cout << "test " << test_count++ << endl;
	...
	get_lumi_weight( Weights_map , "/wk_cms2/cychuang/CMSSW_9_4_2/src/TopCPViolation/selected/info./data_sets_dbl_pre.txt");
	...
	get_path( Data_Set_Path ,"/wk_cms2/cychuang/CMSSW_9_4_2/src/TopCPViolation/selected/info./data_sets_dbl_pre.txt");
	...
	TFile* f7 = new TFile("/wk_cms2/cychuang/CMSSW_9_4_2/src/TopCPViolation/data/beffPlot_QCD_0pt2217.root");		//QCD dominant
		...
		for(int r=0;r<(int)files_map[ Set_name ]->size();r++)
		{
			...
			*/
			...
			for(int entry=0;entry<(int)t_entries;++entry)
			{
				...
				bool pass_sel_mu = Pass_CR_InvSelected_Muon(leptonInfo,idx_Selected_Mu);
				bool pass_sel_el = Pass_CR_InvSelected_Electron(leptonInfo,idx_Selected_El,evtInfo);
	...
	string new_file_name = "CR_inv_" + time + ".root";

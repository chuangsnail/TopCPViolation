...
cout << "test " << test_count++ << endl;
	...
	TH1F* h_min_chi2_mu = new TH1F("h_min_chi2_mu","",500,0.,500.);
	TH1F* h_min_chi2_cor_mu = new TH1F("h_min_chi2_cor_mu","",500,0.,500.);
	TH1F* h_min_chi2_incor_mu = new TH1F("h_min_chi2_incor_mu","",500,0.,500.);
	...
	TH1F* h_min_chi2_el = new TH1F("h_min_chi2_el","",500,0.,500.);
	TH1F* h_min_chi2_cor_el = new TH1F("h_min_chi2_cor_el","",500,0.,500.);
	TH1F* h_min_chi2_incor_el = new TH1F("h_min_chi2_incor_el","",500,0.,500.);
	
	TH1F* h_max_mva_cor_t = new TH1F( "h_max_mva_cor_t","",100,0.,1. );
	TH1F* h_max_mva_incor_t = new TH1F( "h_max_mva_incor_t","",100,0.,1. );

	TH1F* h_min_chi2_cor_t = new TH1F( "h_min_chi2_cor_t","",500,0.,500. );
	TH1F* h_min_chi2_incor_t = new TH1F( "h_min_chi2_incor_t","",500,0.,500. );
	...
	/*
	inputVars.push_back("hadb_Pt");
	inputVars.push_back("hadb_Phi");
	inputVars.push_back("hadb_Eta");
	inputVars.push_back("lepb_Pt");
	inputVars.push_back("lepb_Eta");
	inputVars.push_back("lepb_Phi");
	...
	inputVars.push_back("lepblep_absdelPt");
	...
	inputVars.push_back("lepb_deepcsv_v");
	*/
		...
		for(int r=0;r<(int)files_map[ Set_name ]->size();r++)
		{
			...
			*/
			...
			for(int entry=0;entry<(int)t_entries;++entry)
			{
				...
				double MAX_mva_value = 0.;				//ANN's value is 0~1
				...
				for(int B=0;B<(int)sel_b_jets.size();B++)
				{
					...
					for(int J1=0;J1<(int)sel_jets.size();J1++)
					{
						...
						{
							/*
							var[0] = jetInfo.Pt[ sel_b_jets.at(B) ];
							var[1] = jetInfo.Phi[ sel_b_jets.at(B) ];
							var[2] = jetInfo.Eta[ sel_b_jets.at(B) ];
							var[3] = jetInfo.Pt[ sel_b_jets.at(lB) ];
							var[4] = jetInfo.Eta[ sel_b_jets.at(lB) ];
							var[5] = jetInfo.Phi[ sel_b_jets.at(lB) ];
							var[6] = jetInfo.Pt[ sel_jets.at(J1) ] + jetInfo.Pt[ sel_jets.at(J2) ];
							var[7] = fabs(jetInfo.Pt[ sel_jets.at(J1) ] - jetInfo.Pt[ sel_jets.at(J2) ] );
							var[8] = fabs(jetInfo.Eta[ sel_jets.at(J1) ] - jetInfo.Eta[ sel_jets.at(J2) ] );
							var[9] = TVector2::Phi_mpi_pi( jetInfo.Phi[ sel_jets.at(J1) ] - jetInfo.Phi[ sel_jets.at(J2) ] );
							var[10] = fabs(leptonInfo.Pt[ idx_Selected_Lep ] - jetInfo.Pt[ sel_b_jets.at(lB) ]);
							var[11] = fabs(leptonInfo.Eta[ idx_Selected_Lep ] - jetInfo.Eta[ sel_b_jets.at(lB) ] );
							var[12] = TVector2::Phi_mpi_pi( leptonInfo.Phi[ idx_Selected_Lep ] - jetInfo.Phi[ sel_b_jets.at(lB) ] );
							var[13] = leptonInfo.Pt[ idx_Selected_Lep ] + jetInfo.Pt[ sel_b_jets.at(lB) ];
							var[14] = jetInfo.pfDeepCSVJetTags_probb[ sel_b_jets.at(B) ] + jetInfo.pfDeepCSVJetTags_probbb[ sel_b_jets.at(B) ];
							var[15] = jetInfo.pfDeepCSVJetTags_probb[ sel_b_jets.at(lB) ] + jetInfo.pfDeepCSVJetTags_probbb[ sel_b_jets.at(lB) ];
							*/
							...
							if( tmp_mva_value >= MAX_mva_value )
							{
								...
								mva_hadb = sel_b_jets.at(B);
								mva_j1 = sel_jets.at(J1);
								mva_j2 = sel_jets.at(J2);
							}
						}
					}
				}
			...
			cout << "c_check_lep" << mvamgr.c_check_lep << endl;
			cout << "c_check_bb" << mvamgr.c_check_bb << endl;
			cout << "c_check_jj" << mvamgr.c_check_jj << endl;
	...
	string new_file_name = "cor_mva_chi2_t05_" + time + ".root";
	...
	cout << "cor_mva ratio : " << (double)cor_mva/s_weight << endl; 	
   	...
   	cout << "cor_mva_el ratio : " << (double)cor_mva_el/s_weight_el << endl << endl; 	

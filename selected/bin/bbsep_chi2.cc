/**************************************************************************************
 *
 *	File Name : bbsep_chi2.cc
 *	Description : chi2's bbsep
 *	Date : 191024~
 *	Author : Chen-Yu Chuang
 *
 ****************************************************************************************/
#include "TopCPViolation/selected/interface/Comp_DataMC.h"
//#include "/wk_cms2/cychuang/CMSSW_9_4_2/src/wk_mva/train_11/weights/TMVAClassification_MLP.class.C"
...
int test_count = 0;
			...
			*/
			...
			for(int entry=0;entry<(int)t_entries;++entry)
			{
				...
				*/
				...
				int chi2_hadb = -1, chi2_j1 = -1, chi2_j2 = -1, chi2_lepb = -1;
				...
				chi2_lepb = sel_b_jets.at(1);
				
				TLorentzVector p_chi2_j1, p_chi2_j2, p_chi2_hadb, p_chi2_lepb, p_chi2_lep;
				...
				p_chi2_lepb.SetPxPyPzE(jetInfo.Px[chi2_lepb],jetInfo.Py[chi2_lepb],jetInfo.Pz[chi2_lepb],jetInfo.Energy[chi2_lepb]);
				p_chi2_lep.SetPxPyPzE(leptonInfo.Px[idx_Selected_Lep],leptonInfo.Py[idx_Selected_Lep],leptonInfo.Pz[idx_Selected_Lep],leptonInfo.Energy[idx_Selected_Lep]);

				double chi2_tmass = ( p_chi2_j1 + p_chi2_j2 + p_chi2_hadb ).M();
				double chi2_lep_t_mass = ( p_chi2_lepb + p_chi2_lep ).M();
				...
				//
				//
				int had_b = chi2_hadb;	//which we chose in jjb (reconstructed top quark)	
				...
				switch(bbType)
				{
					case correct:
						...
						h_correct_t->Fill( chi_square_value, weight );
						h_lt_correct[channel]->Fill( chi2_lep_t_mass, weight );
						h_lt_correct_t->Fill( chi2_lep_t_mass, weight );
						break;
					case charge_mis:
						h_charge_mis[channel]->Fill(chi_square_value,weight);
						h_charge_mis_t->Fill(chi_square_value,weight);
						h_lt_charge_mis[channel]->Fill( chi2_lep_t_mass ,weight);
						h_lt_charge_mis_t->Fill( chi2_lep_t_mass ,weight);
						break;
					case mistag:
						h_mistag[channel]->Fill(chi_square_value,weight);
						h_mistag_t->Fill(chi_square_value,weight);
						h_lt_mistag[channel]->Fill( chi2_lep_t_mass ,weight);
						h_lt_mistag_t->Fill( chi2_lep_t_mass ,weight);
						break;
					case no_match:
						no_match_number++;
						break;
	...
	string new_file_name = "bbsep_chi2_" + time + ".root";
	...
	cout << "no-match no. : " << no_match_number << endl;

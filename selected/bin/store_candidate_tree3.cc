/**************************************************************************************
 *
 *	File Name : store_candidate_tree3.cc
...
int main(int argc,char* argv[])
{
	...
	*/
	...
	//correct->Branch("chi2_value",&c_chi2_value,"chi2_value/D");
	correct->Branch("hadb_probb",&c_hadb_probb,"hadb_probb/D");
	correct->Branch("hadb_probbb",&c_hadb_probbb,"hadb_probbb/D");
	...
	//incorrect->Branch("chi2_value",&i_chi2_value,"chi2_value/D");
	incorrect->Branch("hadb_probb",&i_hadb_probb,"hadb_probb/D");
	incorrect->Branch("hadb_probbb",&i_hadb_probbb,"hadb_probbb/D");
                ...
                for(int j=0;j<(int)sel_jets.size();j++)
                {
                    for(int k=j+1;k<(int)sel_jets.size();k++)
                    {
                        if( ( sel_b_jets.at(i) == cor_b && sel_jets.at(j) == cor_j1 && sel_jets.at(k) == cor_j2 ) || ( sel_b_jets.at(i) == cor_b && sel_jets.at(j) == cor_j2 && sel_jets.at(k) == cor_j1) )
                        {
							...
							c_hadblepb_delPt = jetInfo.Pt[ cor_b ] - jetInfo.Pt[ leptonic_b ];
							//c_chi2_value = chi_2_value( jetInfo, sel_jets, sel_b_jets, cor_j1, cor_j2, cor_b );
							c_hadb_probb = jetInfo.pfDeepCSVJetTags_probb[ cor_b ];
							c_hadb_probbb = jetInfo.pfDeepCSVJetTags_probbb[ cor_b ];
							...
							c_top_mass = ( p_mva_j1 + p_mva_j2 + p_mva_hadb ).M();
							c_w_mass = ( p_mva_j1 + p_mva_j2 ).M();
                        ...
                        else	//incorrect combination
                        {
							...
							i_hadblepb_delPt = jetInfo.Pt[ sel_b_jets.at(i) ] - jetInfo.Pt[ leptonic_b ];
							//i_chi2_value = chi_2_value( jetInfo, sel_jets, sel_b_jets, j, k, i );
							i_hadb_probb = jetInfo.pfDeepCSVJetTags_probb[ sel_b_jets.at(i) ];
							i_hadb_probbb = jetInfo.pfDeepCSVJetTags_probbb[ sel_b_jets.at(i) ];
							...
							i_top_mass = ( p_mva_j1 + p_mva_j2 + p_mva_hadb ).M();
							i_w_mass = ( p_mva_j1 + p_mva_j2 ).M();

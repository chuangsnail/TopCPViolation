...
int main(int argc,char* argv[])
{
	...
	TFile* f7 = new TFile("/wk_cms2/cychuang/CMSSW_9_4_13/src/TopCPViolation/data/beffPlot_TT_0pt8484.root");
...
*/
	...
	for(int k=0;k<(int)files_map.size();k++)
	{
		...
		for(int r=0;r<(int)files_map[ Set_name ]->size();r++)
		{
			...
			for(int entry=0;entry<(int)t_entries;++entry)
			{
				...
				if(channel == "mu")
				{	h_mu.at(k)->Fill(lep_t_mass,weight);	}
				if(channel == "el")
				{	h_el.at(k)->Fill(lep_t_mass,weight);	}
	...
	string new_file_name = "SR_lb_" + time + ".root";

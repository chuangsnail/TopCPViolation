                ...
                for(int j=0;j<(int)sel_jets.size();j++)
                {
                    for(int k=j+1;k<(int)sel_jets.size();k++)
                    {
                        if( ( sel_b_jets.at(i) == cor_b && sel_jets.at(j) == cor_j1 && sel_jets.at(k) == cor_j2 ) || ( sel_b_jets.at(i) == cor_b && sel_jets.at(j) == cor_j2 && sel_jets.at(k) == cor_j1) )
                        {
							...
							c_whadb_absdelEta = fabs( ( p_mva_j1 + p_mva_j2 ).Eta() - p_mva_hadb.Eta() );
							c_whadb_delPhi = TVector2::Phi_mpi_pi( ( p_mva_j1 + p_mva_j2 ).Phi() - p_mva_hadb.Phi() );
                            
							c_whadb_sumPt = (p_mva_j1 + p_mva_j2).Pt() + p_mva_hadb.Pt();
                            c_whadb_delPt = (p_mva_j1 + p_mva_j2).Pt() - p_mva_hadb.Pt();
                        ...
                        else	//incorrect combination
                        {
							...
							i_whadb_absdelEta = fabs( ( p_mva_j1 + p_mva_j2 ).Eta() - p_mva_hadb.Eta() );
							i_whadb_delPhi = TVector2::Phi_mpi_pi( ( p_mva_j1 + p_mva_j2 ).Phi() - p_mva_hadb.Phi() );

                           	i_whadb_sumPt = (p_mva_j1 + p_mva_j2).Pt() + p_mva_hadb.Pt();
                            i_whadb_delPt = (p_mva_j1 + p_mva_j2).Pt() - p_mva_hadb.Pt();

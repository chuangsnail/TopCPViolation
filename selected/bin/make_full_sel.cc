...
{
	string data_sets_name[7] = { "TT","DY","WJets","VV","ST","QCD","Data_SM", "Data_SE" };
	
	//*********************About path***********************//
	
	//Load in the pre_sel files' path
	...
	Data_Set_Path[data_sets_name[6]] = &Data_SM;		Data_Set_Path[data_sets_name[7]] = &Data_SE;
	...
	//insert lepton Scale Factors' TH2F*
	...
	for(int k=0;k<(int)Data_Set_Path.size();k++)
	{
		double weight = 1.;
		bool is_data = false;
		...
		for(int r=0;r<(int)[ Set_name ]->size();r++)
		{
			...
			*/
			...
			for(int entry=0;entry<(int)t_entries;++entry)
			{
				...
				if( entry%50000 == 0 )
				{	
					cout << "At percentage : " << (double)entry/t_entries * 100 << " %" << endl;
				}
	...
	string new_file_name = "re_btag2_" + time + ".root";

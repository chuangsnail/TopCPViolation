...
int main(int argc,char* argv[])
{
	...
	char d2[500] = "/wk_cms2/cychuang/pre_sel_file/pre_sel_TT/dir_002/pre_sel_TT_3.root" ;	
	
	
	vector<File> files_TT;
	File f_TT_1(TT[0]);
	File f_TT_2(TT[1]);
	files_TT.push_back(f_TT_1);
	files_TT.push_back(f_TT_2);
	...
	File f_Data_1(d1);
	File f_Data_2(d2);
	files_Data.push_back(f_Data_1);
	files_Data.push_back(f_Data_2);
	...
	for(int k=0;k<3;k++)
	{
		...
		for(int r=0;r<(int)files_map[ Set_name ]->size();r++)
		{
			...
			if(!is_data){
				printf("\ntest2");
				if(r!=(int)files_map[ Set_name ]->size()-1)
				{
					printf("\ntest3");

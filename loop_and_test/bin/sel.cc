...
int main(int argc,char* argv[])
{
	...
	*/
	...
	h_mu.push_back(h_DY_mu);
	h_mu.push_back(h_WJets_mu);
	h_mu.push_back(h_VV_mu);
	h_mu.push_back(h_ST_mu);
	//h_mu.push_back(h_QCD_mu);
	h_mu.push_back(h_Data_mu);

	vector<TH1F*> h_el;
	h_el.push_back(h_TT_el);
	h_el.push_back(h_DY_el);
	h_el.push_back(h_WJets_el);
	h_el.push_back(h_VV_el);
	h_el.push_back(h_ST_el);
	//h_el.push_back(h_QCD_el);
	...
	vector<File> files_DY;
	vector<File> files_WJets;
	vector<File> files_VV;
	vector<File> files_ST;
	//vector<File> files_QCD;
...
*/
	...
	files_map[data_sets_name[1]] = &files_DY;
	files_map[data_sets_name[2]] = &files_WJets;
	files_map[data_sets_name[3]] = &files_VV;
	files_map[data_sets_name[4]] = &files_ST;
	//files_map[data_sets_name[5]] = &files_QCD;
	

	map< string, vector<double>* > Weights_map;
	Weights_map[data_sets_name[0]] = &w_TT;
	Weights_map[data_sets_name[1]] = &w_DY;
	Weights_map[data_sets_name[2]] = &w_WJets;
	Weights_map[data_sets_name[3]] = &w_VV;
	Weights_map[data_sets_name[4]] = &w_ST;
	//Weights_map[data_sets_name[5]] = &w_QCD;
	...
	File f_Data(Data_SM[0]);
	for(int i=1;i<(int)Data_SM.size();i++){
		f_Data.AddFile(Data_SM[i]);
	}
	for(int i=0;i<(int)Data_SE.size();i++){
		f_Data.AddFile(Data_SE[i]);
	}
	...
	for(int k=0;k<(int)files_map.size();k++)
	{
		...
		//loop around in a kind of data set that is files_map[ data_sets_name[k] ]
		for(int r=0;r<(int)files_map[ Set_name ]->size();r++)
		{
			...
			printf("\nThe file now dealing with is under the sets %s the %d one.",Set_name.c_str(),r+1);
			...
			for(int entry=0;entry<(int)t_entries;++entry)
			{
				...
				if(is_mu_channel && !is_el_channel)
				{
					...
					bool pass_delR = Pass_delta_R(leptonInfo,idx_Selected_Mu,jetInfo,sel_jets,&sel_jets_no);
					if( !pass_delR )
					{	continue;	}
					...
					if(!is_data)
					{
						...
						weight = weight * GetLepSF(&leptonInfo,h_tightMuIDSF,idx_Selected_Mu);
					...
					//printf("\n Selected Top Mass of entry-%d in %s the %d one set: %.10f",entry,Set_name.c_str(),r+1,sel_t_mass);
				...
				{	
					if(!pass_sel_jet)
					{	continue;	}
					bool pass_delR = Pass_delta_R(leptonInfo,idx_Selected_El,jetInfo,sel_jets,&sel_jets_no);
					if(!pass_delR)
					{	continue;	}
					bool pass_sel_b = Pass_Selected_b_Jet(jetInfo,sel_jets,&sel_jets_no,sel_b_jets);
					if(!pass_sel_b)
					{	continue;	}
						...
						{
							//do the b-tag reweighting scale factor
							int b_1_idx = sel_b_jets[0];
							int b_2_idx = sel_b_jets[1];
							...
							//do the tight-Electron ID scale factor to make MC more match to data
							weight = weight * GetLepSF(&leptonInfo,h_tightElIDSF,idx_Selected_El);
			...
			printf("\nAnd the Selected entries of this sets %s the %d one are : %d",Set_name.c_str(),r+1,selected_entries);
		}	//end of r for-loop
	}		//end of k for-loop
